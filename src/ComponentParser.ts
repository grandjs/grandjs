...
class ComponentParserClass {
  ...
  render(Component: Function, data: OptionalObject) {
    let component = Component(data);
    let result = this.parser(component, data);
    return result;
  }
  importJsx<T>(path: string):T | any {
    ...
    if (!foundFile) {
      const importedFile = importJsx(path, {
        pragma: "ComponentParser.createElement" || "this.createElement",
      });
...
const ComponentParser = new ComponentParserClass();
