    ...
    constructor() {
        ...
        this.Router = class Router {
            ...
            init() {
                ...
                if (req.method.toLowerCase() == "get") {
                    ...
                    if (this.inited == false) {
                        ...
                        this.getRouters.forEach((router) => {
                            ...
                            let values = [];
                            routePattern.ast.map((tag) => { values.push(tag.value) });
                            values = values.join("");
                    ...
                    this.finalGetRouters.forEach((r) => {
                        ...
                        if (matchedFinal) {
                            let values = [];
                            r.ast.map((tag) => { values.push(tag.value) });
                            values = values.join("");
                ...
                } else if (req.method.toLowerCase() == "post" || req.method.toLowerCase() == "put" || req.method.toLowerCase() == "delete" || req.method.toLowerCase() == "patch") {
                    ...
                    if (matchedURL) {
                        if (matchedURL.method.toLowerCase() == "post" || matchedURL.method.toLowerCase() == "put" || matchedURL.method.toLowerCase() == "delete" || matchedURL.method.toLowerCase() == "patch") {
                            res.statusCode = 200;
        ...
        function app(req, res) {
            ...
            function chooseHandlers() {
                ...
                if (matchedReq) {
                    ...
                    let ur = config.extenstions.find(ext => path.extname(pathname) == ext);
                    if (ur) {
                        return helpers.staticFiles(req, res, req.pathname, self.staticFolder, self.cacheControl).catch((src) => {
                            res.writeHead(404);
