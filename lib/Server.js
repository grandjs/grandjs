...
class BaseServer {
    ...
    initServer(cb) {
        ...
        let callBack = (req, res) => {
            ...
            let pathToSkip = request.pathname;
            if (path_1.default.extname(pathToSkip)) {
                pathToSkip = request.pathname.replace(/\.[^.]*$/, "");
            }
            if (foundStaticRoute && path_1.default.extname(request.pathname)) {
    ...
    static(options) {
        ...
        let route = {
            ...
            handler: helpers_1.default.serveAssets.bind(Object.assign({}, this, { staticFolder: { url: options.url, path: options.path }, chooseHandler: this.chooseHandler })),
            assetsPath: true
        };
        this.statics.push(new Route_1.default(route, "/"));
    }
}
