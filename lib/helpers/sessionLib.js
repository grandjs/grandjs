...
class Session {
    ...
    set(sessionName, obj, sessionOptions) {
            let path = sessionOptions ? sessionOptions.path || "/" : "/";
            let res = this.res;
            let req = this.req;
            let cookiesOptions = this.cookieOptions !== undefined && typeof this.cookieOptions === "object" ? this.cookieOptions : {};
            let cookieExpires = sessionOptions ? sessionOptions.expires : false || cookieOptions.expires || {};
            ...
            if (cookieExpires !== false) {
                res.setHeader("set-cookie", `${sessionName}=${sessionId};path=${path};${expiresDays && minutes !== undefined ? `expires=${expires}`:""};`);
        } else if (cookieExpires === false) {
            res.setHeader("set-cookie", `${sessionName}=${sessionId};path=${path};`);
        }
