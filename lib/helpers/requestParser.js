...
class RequestParser {
    ...
    handler(matchedURL) {
            ...
            if (headers["content-type"]) {
                ...
                if (requestParser) {
                    requestParser.bind(this)(matchedURL);
                } else {
                    this.req.data = {};
                    this.req.files = {};
                }
                ...
                this.req.data = {};
                this.req.files = {};
                return MiddeWares.next(matchedURL, this.req, this.res);
            }
        }
        // method to parse form url encode
    parseFormUrlEncode(matchedURL) {
            let headers = this.req.headers;
            let comingData = "";
            let data = {};
            let files = {};
            ...
            this.req.on("end", () => {
                ...
                MiddeWares.next(matchedURL, this.req, this.res);
            });
        }
        // method to parse formdata request
    parseFormData(matchedURL) {
                ...
                part.on("end", () => {
                    ...
                    if (part.filename) {
                        ...
                        let file = {
                            ...
                            mimeType: part.headers["content-type"].split("/")[0]
                        };
